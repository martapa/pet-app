  ...
  render() {
    return (
      <>
          ...
          {({ handleSubmit, handleChange, values, touched, errors }) => (
            <Form inline onSubmit={handleSubmit}>
              <FormControl
                ...
                className={
                  this.state.urlPath === '/'
                    ? 'mr-sm-2 search-form-light'
                    : 'mr-sm-2 search-form-dark'
                }
                value={values.search}
        ...
        {this.state.serverErrors && <p>{this.state.serverErrors}</p>}
      </>
    );
  }
}
...
export default withRouter(connect(mapStateToProps, { getDogsNearYou })(Search));
